...
router.get('/personnelbyid', asyncHandler(async (req, res) => {
...
router.get('/ninumber', asyncHandler(async (req, res) => {
  ...
  const { Personnel } = getModels(req);
  const personnel = await Personnel.findOne({ "niDetails.nationInsuranceNumber": niNumber });
  if (!personnel) throw new Error('Personnel not found');
  res.json(personnel);
}));
...
// Server-side implementation of getPersonnelTypeForDate from Rota.jsx
...
const rateCardFinder = async (RateCard, date, serviceWeek, service, personnel) => {
  return await RateCard.findOne({
    serviceWeek,
    serviceTitle: service,
    vehicleType: getPersonnelTypeForDate(personnel, date),
    active: true,
  });
};
...
router.put('/newupdate/:id', upload.any(), asyncHandler(async (req, res) => {
  const { Personnel, Notification, User, DayInvoice, WeeklyInvoice, Installment, RateCard } = getModels(req);
  const round2 = (num) => +parseFloat(num || 0).toFixed(2);
  ...
  if (personnelData.companyVatDetails)
    personnelData.companyVatDetails = parseJsonField(personnelData, 'companyVatDetails');
  if (personnelData.typeOfPersonnel === 'Own Vehicle') {
    personnelData.ownVehicleInsuranceNA = parseJsonField(personnelData, 'ownVehicleInsuranceNA');
  }
  if (personnelData.customTypeOfPersonnel) {
    personnelData.customTypeOfPersonnel = parseJsonField(personnelData, 'customTypeOfPersonnel');
  }
  personnelData.typeOfPersonnelTrace = parseJsonField(personnelData, 'typeOfPersonnelTrace');
  ...
  const updateFields = {
    ...
    bankDetails: personnelData.bankDetails,
    sortCode: personnelData.sortCode,
    drivingLicenseDetails: personnelData.drivingLicenseDetails,
    activeStatus: personnelData.activeStatus,
    expiredReasons: personnelData.expiredReasons,
    addedBy: personnelData.addedBy,
    passportDetails: personnelData.passportDetails,
    rightToWorkDetails: personnelData.rightToWorkDetails,
    siteSelection: personnelData.siteSelection,
    ecsDetails: personnelData.ecsDetails
  };
  ...
  const vatDates = vatChanged ? [
    ...
    personnelData.companyVatDetails?.companyVatEffectiveDate ? new Date(personnelData.companyVatDetails.companyVatEffectiveDate) : null,
  ].filter(Boolean) : [];

  const customTypeDates = customTypeChanged ? [
    ...Object.keys(originalCustomTypeOfPersonnel || {}).map((d) => new Date(d.split('/').reverse().join('-'))),
    ...Object.keys(personnelData.customTypeOfPersonnel || {}).map((d) => new Date(d.split('/').reverse().join('-'))),
  ].filter(Boolean) : [];

  const traceDates = traceChanged ? [
    ...originalTypeOfPersonnelTrace.map((t) => {
      const [day, month, year] = t.timestamp.split('/');
      return new Date(`${year}-${month}-${day}`);
    }),
    ...personnelData.typeOfPersonnelTrace.map((t) => {
      const [day, month, year] = t.timestamp.split('/');
      return new Date(`${year}-${month}-${day}`);
    }),
  ].filter(Boolean) : [];
  ...
  for (const invoice of affectedDayInvoices) {
    ...
    const newVehicleType = getPersonnelTypeForDate(tempPersonnel, invDate);

    // Update main service
    const mainRateCard = await rateCardFinder(RateCard, invDate, invoice.serviceWeek, invoice.mainService, tempPersonnel);
    ...
    if (invoice.additionalServiceDetails?.service && invoice.additionalServiceApproval === 'Approved') {
      const additionalRateCard = await rateCardFinder(RateCard, invDate, invoice.serviceWeek, invoice.additionalServiceDetails.service, tempPersonnel);
  ...
  const isVatApplicable = (date, personnelData) => {
    return (
      ...
      (personnelData.companyVatDetails?.companyVatNo && date >= new Date(personnelData.companyVatDetails.companyVatEffectiveDate))
    );
  };
...
router.put('/:id', asyncHandler(async (req, res) => {
  ...
  if (req.body.typeOfPersonnel === 'Own Vehicle') {
    updateFields.ownVehicleInsuranceNA = parseJsonField(req.body, 'ownVehicleInsuranceNA');
  }

  const updatedPersonnel = await Personnel.findByIdAndUpdate(req.params.id, { $set: updateFields }, { new: true });

  if (personnel.email !== updatedPersonnel.email || personnel.firstName !== updatedPersonnel.firstName || personnel.lastName !== updatedPersonnel.lastName) {
    await User.findOneAndUpdate(
      { user_ID: updatedPersonnel.user_ID },
      { firstName: updatedPersonnel.firstName, lastName: updatedPersonnel.lastName, email: updatedPersonnel.email }
    );
  }

  if (personnel.siteSelection !== updatedPersonnel.siteSelection) {
    const notification = {
      personnel: updatedPersonnel._id,
      site: [personnel.siteSelection, updatedPersonnel.siteSelection],
      changed: 'personnels',
  ...
  sendToClients(req.db, { type: 'personnelUpdated' });
  res.json(updatedPersonnel);
}));
...
router.post('/togglePersonnel/:id', asyncHandler(async (req, res) => {
